...
def_fixed_gamma <- function(data, sigmoid, core, gamma=0.05,par=NULL, fn=NULL, gr=NULL, ...,
  ...
  fit <- NULL
  if(default_fn){
    fit <- tryCatch({stats::optim(par=par, fn=fn, gr=gr, gamma, sigmoidf, sigmoidi, coref, corei_x, data, method=method, lower=lower, upper=upper, control=control)})
  }else{
...
fn_def_fixed_gamma <- function(params, gamma, sigmoidf, sigmoidi, coref, corei_x, data){
  lambda <- 1 - sigmoidf(params[1])

  if(lambda<0 || lambda > 0.11 || (gamma+lambda > 1)) {return(Inf)}
  ...
  if(midpoint < min(data$predictor) || midpoint > max(data$predictor)){return(Inf)}
